...
- are called *after* their triggering DB transaction commits, never inside one, since an LLM call is
  slow external I/O and must not hold a transaction (and its row locks) open
