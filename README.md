...
- [docs/email-setup.md](docs/email-setup.md): Resend account/API key setup, env vars, and the
  shared-sender delivery limitation
...
- `RESEND_API_KEY` / `EMAIL_FROM`: optional; without these, emails are logged as `FAILED` in
  `NotificationLog` and retried, but nothing crashes (see `docs/email-setup.md`)
...
Transactional email, including account creation, booking confirmation, a separate new-booking notice to
the doctor (with the AI pre-visit summary), post-visit summary, cancellation, leave notice, and
appointment/medication reminders, is sent via [Resend](https://resend.com)'s HTTP API
